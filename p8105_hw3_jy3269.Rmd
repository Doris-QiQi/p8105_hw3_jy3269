...
*  Overall, `ny_noaa` contains all the information from each weather station for each day and the date ranges from  of January 1st 1981 to December 31st 2010. The data contains a lot of missing values. 


The missing value problem:
*  There are a total of `r nrow(ny_noaa)-nrow(no_missing)` rows that contains missing data, which is `r nrow(no_missing)/nrow(ny_noaa)*100`% of the total observations. 
*  It means that nearly half of the `ny_noaa` data set have missing values.


Therefore, missing data is a big issue. However, since the observation left is `r nrow(no_missing)`, which is big enough, removing the missing rows will not have a big problem as there are still enough data to draw conclusions
